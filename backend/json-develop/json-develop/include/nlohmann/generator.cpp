...
public:
    ...
    double capacity;
    double speed;
    Vehicle(string type, double rent, double capacity, double speed)
    {
        this->type = type;
        this->rent = rent;
        this->capacity = capacity;
...
int main()
{
    ...
    */
    ...
    vector<Vehicle> vehi;
    for (const auto &veh : input["vehicle"])
    {
        string type_ = veh["type"].get<string>();
        double rent_ = stod(veh["rent"].get<string>());
        double capacity_ = stod(veh["capacity"].get<string>());
        double speed_ = stod(veh["speed"].get<string>());
        Vehicle v(type_, rent_, capacity_, speed_);
        vehi.push_back(v);
    }
    Vehicle v1 = vehi[0]; // Type 1: Medium capacity, higher speed
    Vehicle v2 = vehi[1]; // Type 2: Higher capacity, lower speed

    // Hardcoded farm data
    Farm farm;
    for (const auto &farms_ : input["farm"])
    {
        int id_ = stoi(farms_["id"].get<string>());
        string crop_ = farms_["crop"].get<string>();
        double perish = stod(farms_["perish_rate"].get<string>());
        double quant = stod(farms_["quant"].get<string>());
        Farm fr(id_, crop_, perish, quant);
        farm = fr;
    }

    map<int, Center> centerMap;
    for (const auto &cent : input["center"])
    {
        int id_ = stoi(cent["id"].get<string>());
        string crop_ = cent["type"].get<string>();
        double dem = stod(cent["demand"].get<string>());
        double rt = stod(cent["rate"].get<string>());
        double deadl = stod(cent["deadline"].get<string>());
        centerMap[id_] = Center(id_, crop_, dem, rt, deadl);
    }
    ...
    {
        if (v1.capacity > hub.capacity)
            continue;

        for (const auto &[id, center] : centerMap)
        {

            if (center.crop != farm.crop)
            {
                continue;
            }
            ...
            double ncost = cost1 + cost2 - (time1 + time2) * (min(farm.quantity, v1.capacity) - rem) * farm.perish_rate * center.rate + hub.cost;

            double p = q * (min(farm.quantity, v1.capacity) * center.rate -
                            cost2 - cost1 - hub.cost) +
                       rem * center.rate - ncost;
    ...
    for (const auto &hub : hubs)
    {
        ...
        for (const auto &[id, center] : centerMap)
        {
            ...
            double ncost = cost1 + cost2 - (time1 + time2) * (min(farm.quantity, v2.capacity) - rem) * farm.perish_rate * center.rate + hub.cost;
